...
function setupEventListeners() {
  console.log("setupEventListeners ishga tushdi");
...
async function handleLogin(e) {
   console.log("handleLogin boshladi");
  e.preventDefault()
    ...
    .map(
      (product) => `
        <div class="alert-item alert-warning">
            ...
            <strong>${product.name}</strong> - Qolgan: ${product.quantity_grams}g (Minimum: ${product.minimum_quantity}g)
        </div>
    `,
    )
    .join("")
...
function getStockStatus(product) {
  if (product.quantity_grams <= product.minimum_quantity) {
    return "status-low"
  }
...
function openProductModal(productId = null) {
  ...
  if (productId) {
    ...
    document.getElementById("productQuantity").value = product.quantity_grams
...
async function handleProductSubmit(e) {
  ...
  const productData = {
    ...
    quantity_grams: Number.parseInt(document.getElementById("productQuantity").value),
    minimum_quantity: Number.parseInt(document.getElementById("productMinimum").value),
...
function openMealModal(mealId = null) {
  ...
  if (mealId) {
    ...
    meal.ingredients.forEach((ing) => {
      addIngredientForm(ing.product_id, ing.quantity_grams)
    })
  } else {
...
function addIngredientForm(productId = "", quantity = "") {
  const container = document.getElementById("ingredientsList")
  ...
  ingredientDiv.innerHTML = `
        ...
        <div class="form-group">
            <label>Miqdor (g):</label>
            <input type="number" class="ingredient-quantity" min="1" value="${quantity}" required>
        </div>
...
async function handleMealSubmit(e) {
  ...
  document.querySelectorAll(".ingredient-form").forEach((ingredientForm) => {
    ...
    const quantity = Number.parseInt(ingredientForm.querySelector(".ingredient-quantity").value)
...
// Expose functions globally so that HTML onclick can access them
